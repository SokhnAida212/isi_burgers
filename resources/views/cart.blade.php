...
<h1 class="px-4 pt-1 pb-3 text-3xl font-bold">
    <div class="flex flex-row">
        <div class="flex flex-row flex-1">
            <span class="mr-5 self-center"> Mon panier </span>
        ...
        <div class="flex-1 text-right text-2xl self-center">
            @if (!empty(session('cart')))
            <span class="mr-8 "> Total : {{($total)}} FCFA</span>
            @endif
        </div>
    </div>
</h1>
            ...
            <div class="flex flex-col justify-between">
                <div class="flex flex-col items-center bg-white rounded-lg border shadow-md md:flex-row md:max-w-full hover:bg-gray-100">
                    ...
                    <div class="flex flex-col place-content-center px-4 py-3 leading-normal w-4/6">
                        ...
                        <p class="flex font-normal text-gray-700"> Quantité : <b>&nbsp;{{$food['quantity']}}</b> </p>
                        <p class="flex font-normal text-gray-700"> Prix : <b>&nbsp;{{($food['price']*$food['quantity'])}}FCFA &ensp;</b> <span class="opacity-60"> {{ (int) $food['price'] }}FCFA l'unité] <span> </p>
                    </div>
        ...
        <div class="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="p-1 sm:flex sm:items-start">
                    ...
                    <div class="pl-1 sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Êtes-vous sûr de vouloir supprimer cet article de votre commande ?</h3>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form name="remove_form" id="remove_form" method="POST">
                    @csrf
                    @method('delete')
                    <button type="submit" class="closeRemoveModal w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm"> Confirmer </button>
                </form>
...
<div x-show="show" tabindex="0" class="invisible z-40 overflow-auto left-0 top-8 bottom-0 right-0 w-full h-full fixed" id="order-modal">
    <div class="z-50 relative p-3 mx-auto my-0 max-w-full" style="width: 600px;">
        <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden">
            ...
            <div class="px-6 py-3 text-xl border-b font-bold">Passer la commande</div>
            <form method="POST" action="/cart/placeorder" class="px-4 pt-2 pb-4" name="place-order-form" id="place-order-form">
                ...
                <div class="p-6 flex-grow">
                    <div class="mb-4">
                        ...
                        <div class="columns-2 gap-2 w-fit">
                            <div><label> <input type="radio" name="type" id="pickupType" value="Retrait" onclick="showAddressField()" checked> À emporter </label></div>
                            <div><label> <input type="radio" name="type" id="deliveryType" value="Livraison" onclick="showAddressField()"> Livraison </label></div>
                        </div>
                    </div>
                ...
                <div class="px-6 pt-3 border-t">
                    <div class="flex justify-end">
                        <button type="button" class="closeOrderModal bg-gray-700 text-gray-100 rounded px-4 py-2 mr-1">Fermer</Button>
                        <button type="button" class="openPaymentModal bg-green-600 text-white rounded px-4 py-2">Procéder au paiement</Button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="relative px-4 w-full max-w-md h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-6 pt-3 text-center">
                ...
                <h3 class="mb-3 text-lg font-normal text-gray-500">Le paiement est en cours de traitement.</h3>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="relative px-4 w-full max-w-md h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-6 pt-3 text-center">
                ...
                <h3 class="mb-4 text-2xl font-semibold text-gray-600">Paiement réussi !</h3>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function validateForm() {
        ...
        if (x == "") {
            console.log(address);
            address.innerHTML = "L'adresse de livraison ne peut pas être vide"
            return false;
        }
        return true;
    }
